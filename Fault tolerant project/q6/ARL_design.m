function    ARL_design(mu_1, mu_0, sigma_r_min, sigma_r_max, h_min, h_max)
%
...
if(makeplot)
    ...
    subplot(2,1,1),
    plot(h, ceil(timtodetect)), grid, ylabel('samples'),title('Time to detect');
    legend(legend_char_vec,'Location','Best')
 
    subplot(2,1,2)
    semilogy(h, floor(tibfalsealm)), grid, ylabel('samples'), 
    title('Time between false alarms'),xlabel('h')
